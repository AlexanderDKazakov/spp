...
# Tabulated potential https://github.com/openmm/openmm/issues/4381
# http://docs.openmm.org/latest/api-python/generated/openmm.openmm.Continuous1DFunction.html

if __name__ == "__main__":

    gro = app.GromacsGroFile('input.gro')
    top = app.GromacsTopFile('input.top', periodicBoxVectors=gro.getPeriodicBoxVectors(), includeDir=GROMACS_TOP)

    # Creating system
    system = top.createSystem(nonbondedMethod = app.LJPME, nonbondedCutoff = CONFIG['nonBondedCutoff'], constraints = app.HBonds, switchDistance=CONFIG["switchDistance"])
    system.setDefaultPeriodicBoxVectors(
        unit.Quantity((CONFIG['box_l'][0], 0 * unit.angstroms, 0 * unit.angstroms)),
        unit.Quantity((0 * unit.angstroms, CONFIG['box_l'][1], 0 * unit.angstroms)),
        unit.Quantity((0 * unit.angstroms, 0 * unit.angstroms, CONFIG['box_l'][2])),
    )
